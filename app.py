...
def resolve_ticker(raw_symbol):
    ...
    has started blocking cloud-server IPs from its price/quote API with
    401 "Invalid Crumb" errors, which used to take down this whole
    lookup — falling back to yfinance's price only if Twelve Data has no
    data for the symbol. Twelve Data covers the Thai SET exchange too,
    under its own symbol format (no ".BK" suffix), so that gets tried
    separately. Fundamentals (P/E, ROE, etc.) still come from yfinance's
    .info on a best-effort basis: if that's blocked, we still return
    whatever price was found rather than failing the whole lookup.
    Returns (yfinance.Ticker, resolved_symbol, info_dict, price), or
    (None, None, None, None) if no price could be found anywhere.
    """
    ...
    if "." not in symbol:
        yf_candidates.append(f"{symbol}.BK")
...
def build_report(raw_symbol):
    ...
    ticker, resolved, info, price = resolve_ticker(symbol)
    if ticker is None:
        return (
